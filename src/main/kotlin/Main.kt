...
object RepartijaDeRegalos {
    ...
    //val regalosRecibidosObservers: MutableList<regaloRecibidosObservers> = mutableListOf()
    ...
    fun repartir() {
        personas.forEach { persona ->
            ...
            //regalosRecibidosObservers.forEach {it.regaloRecibido(regaloElegido, persona)}
...
class Combineta(val criterioEleccionRegalos: MutableList<CriterioEleccionRegalo>): CriterioEleccionRegalo{
...
//No la hizo pero sería un Null Object Pattern
class CriterioEleccionRegaloVacia: CriterioEleccionRegalo{
    override fun leGusta(regalo: Regalo) = true
}
...
abstract class Regalo(val valor: Int, val marca: String, val codigo: String) {
    ...
    open fun valioso() = ((valor > precioValioso) && valiosoEspecifico())
...
class Experiencia(precio:Int, marca: String, codigo: String,val fechaDeExperiencia: LocalDate): Regalo(precio, marca, codigo) {
    val diaAComparar: java.time.DayOfWeek = DayOfWeek.FRIDAY
...
class RegaloRecibidoPasarAInteresada(): RegaloRecibidoObserver {
    ...
    var valorRegalo = 5000

    override fun regaloRecibido(regalo: Regalo, persona: Persona) {
        if (regalo.valor > limiteMaximo){
            persona.criterioEleccionRegalo = Interesada(valorRegalo)
        }
    }
}
...
* Esta permitido obviarlas por ser un parcial
*
* Si en el enunciado dice "para cada x" - muchas posibilidades de hacer un forEach
* Lo menciona porque por ahí uno piensa en lo siempre, hacer un metodo unitario y no, te está sugiriendo
* recorrer en masa una lista (como lo que había entendido de EstablecerPedido pero a la inversa)
*
* */
